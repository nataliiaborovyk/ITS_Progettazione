...
# colegamento con class Aeroporto, associazione - aerop_citta
	...
	def _aggiorna_elenco_aeroporti(self, link:'aerop_citta._link'):
		''' Aggiorna set(aerop_citta) nella class Citta'''
		# caso di errore
	...
	def _remove_link_aerop_citta(self, link:'aerop_citta._link') -> None:
		''' Rimuove  link dal set(aerop_citta) nella class Citta'''
		# caso di errore
		if link.get_citta() != self:
			raise ValueError(f"Link fornito non riguarda citta {self.get_nome()} ")		
		if link in self._elenco_aeroporti:
...
# collegamento con class Nazione, associazione - cit_naz
	...
	def get_link_cit_naz(self) -> cit_naz._link:
		return self._link_cit_naz

	def get_nazione(self) -> Nazione:
		return self.get_link_cit_naz().get_nazione()

	def crea_link_cit_naz(self, nazione:Nazione) -> None:
		''' Crea link cit_naz da class cit_naz'''
		cit_naz.crea_link_cit_naz(self, nazione)

	def _aggiorna_link_cit_naz(self, link: cit_naz._link) -> None:
		self._link_cit_naz = link
